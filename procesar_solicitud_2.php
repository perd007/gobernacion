...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form2")) {
	  ...
	  $updateSQL = sprintf("UPDATE solicitud SET estado=%s, usuario_proc=%s WHERE id_solicitud=%s",
  ...
  if($_POST["tipo"]=="Constancia de Trabajo"){
  echo "<script type=\"text/javascript\">alert ('Generando Constancia');  location.href='constancia_trabajo.php?cedula=$_POST[cedula]' </script>";
  }
  else{
	if($_POST["tipo"]=="Carnet de Carga Familiar"){
  echo "<script type=\"text/javascript\">alert ('Generando Carnet');  location.href='carnet_familiar.php?cedula=$_POST[cedula]' </script>";
  }else{
...
mysql_select_db($database_BD, $BD);
$query_solicitud = "SELECT * FROM solicitud where empleado='$cedula' and id_solicitud='$id'";
...
<body>
<form action="<?php echo $editFormAction; ?>" target="_blank" method="post" name="form1" id="form1">
  <table align="center" class="bordes">
    <tr valign="baseline">
      <td colspan="2" align="center" bgcolor="#a4e237" class="negrita">Procesar Solicitud de Documentos</td>
    </tr>
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      <td colspan="2" align="center" nowrap="nowrap" bgcolor="#a4e237"><input name="button" type="submit" class="negrita" id="button" value="Procesar" /></td>
    </tr>
  </table>
  <input type="hidden" name="MM_insert" value="form2" />
  <input type="hidden" name="solicitud" value="<?=$_GET['id']?>" />
   <input type="hidden" name="cedula2" value="<?=$cedula?>" />
   <input type="hidden" name="tipo" value="<?=$row_solicitud['tipo']?>" />
</form>
</body>
</html>
